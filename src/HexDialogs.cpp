...
void CopyAsDialog::Copy( void ){
	...
	if( not select->IsState( select->SELECT_FALSE ) ) {
		...
		else if( chcCopyAs->GetSelection() == 2){//HTML
// TODO (death#1#): Implement EndianSwaper
			wxBell();
			}
			...
			bool bigEndianSwapReq = chkBigEndian->GetValue() not_eq wxIsPlatformLittleEndian();
			if (bigEndianSwapReq) wxBell();
// TODO (death#1#): Implement EndianSwaper
